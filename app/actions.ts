...
import { z } from "zod";
import { costera } from "@/lib/costera-provider";


// Create a Costera backend instance
...
export async function generateQuery(
  ...
  try {
    ...
    When using DATE_TRUNC or INTERVAL, only use standard units like 'day', 'week', 'month', 'year'. Do NOT use 'quarter'.
...
export const generateChartConfig = async (
  ...
  try {
    ...
    const result = await generateObject({
      // model: costera('costera/chat-model'),
      model: openai('gpt-4o'),
      schema: configSchema,
